...
| OCR is an assistant, not an automatic converter | OCR can misread exam data; hosted AI inference adds privacy, cost and outage risk | Free browser-side OCR proposes editable rows without uploading the image; organizers must verify every value before saving | Improve local preprocessing and table layout recovery; add private R2 for unsupported complex images |
